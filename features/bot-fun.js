                ...
                if (taggedUser !== null && taggedUser.id !== message.author.id) {
                    firebaseDatabase.child('reputation/' + message.author.id + '/vote_cooldown_timestamp').once('value').then(snap => {
                        ...
                        if (cooldown < Date.now()) {
                            ...
                            message.reply("Please wait " + (Math.round((cooldown - Date.now()) / cooldownMinute)/24) + ":" + (Math.round((cooldown - Date.now()) / cooldownMinute)%24) + " minutes before you can vote again!")
                        }
                    })
                    message.delete()
